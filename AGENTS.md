...
- `fsagent/cli.py`：命令行入口和 CLI 输出格式。
...
- 优先沿用现有模块边界；不要把 API、runtime、CLI、frontend 的职责混在一起。
...
- CLI 中对应参数为 `--trust-project-mcp`；API 中对应字段为 `trustProjectMcp`。
...
- 修改 CLI 参数或输出时，检查 `fsagent/tests/test_cli.py`。
